...
def train_model(dataloaders, model, mode_name, criterion, optimizer, scheduler, num_epochs, device):
...
def run_resnet(mode_name, device, dataloaders, nepocs, nclasses):
    ...
    model_conv, epoch_losses_train_and_valid = train_model(dataloaders, model_conv, mode_name, criterion, optimizer_conv, exp_lr_scheduler, num_epochs=nepocs, device=device)
...
def run_my_net(mode_name, device, dataloaders, nepocs, nclasses):
    class Net(nn.Module):
        def __init__(self):
...
mode_name = "resnet-18"
epoch_losses_train_and_valid, epoch_loss_test, epoch_acc_test, label_list, pred_list = run_resnet(mode_name, device, dataloaders, nepocs, nclasses)
visualize(mode_name, epoch_losses_train_and_valid, epoch_loss_test, epoch_acc_test, label_list, pred_list)
# my net
mode_name = "my net"
nepocs=1
# epoch_losses_train_and_valid, epoch_loss_test, epoch_acc_test, label_list, pred_list = run_my_net(mode_name, device, dataloaders, nepocs, nclasses)
# visualize(mode_name, epoch_losses_train_and_valid, epoch_loss_test, epoch_acc_test, label_list, pred_list)